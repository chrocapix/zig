...
pub fn __expm1h(a: f16) callconv(.c) f16 {
    ...
    return @floatCast(expm1f(a));
}

fn expm1f(x_: f32) callconv(.c) f32 {
    if (math.isNan(x_))
        return math.nan(f32);
...
fn expm1(x_: f64) callconv(.c) f64 {
    if (math.isNan(x_))
        return math.nan(f64);
...
pub fn expm1l(x: c_longdouble) callconv(.c) c_longdouble {
    switch (@typeInfo(c_longdouble).float.bits) {
        16 => return __expm1h(x),
        32 => return expm1f(x),
        64 => return expm1(x),
        80 => return __expm1x(x),
        128 => return expm1q(x),
...
test "expm1_128.special" {
    try expect(math.isPositiveInf(expm1q(math.inf(f128))));
    try expect(expm1q(-math.inf(f128)) == -1.0);
    try expect(math.isNan(expm1q(math.nan(f128))));
}
