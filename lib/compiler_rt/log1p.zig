...
comptime {
    @export(&__log1ph, .{ .name = "__log1ph", .linkage = common.linkage, .visibility = common.visibility });
    @export(&log1pf, .{ .name = "log1pf", .linkage = common.linkage, .visibility = common.visibility });
    @export(&log1p, .{ .name = "log1p", .linkage = common.linkage, .visibility = common.visibility });
    @export(&__log1px, .{ .name = "__log1px", .linkage = common.linkage, .visibility = common.visibility });
    if (common.want_ppc_abi) {
        @export(&log1pq, .{ .name = "log1pf128", .linkage = common.linkage, .visibility = common.visibility });
    }
...
pub fn __log1px(x: f80) callconv(.c) f80 {
    // TODO: more efficient implementation
...
pub fn log1pl(x: c_longdouble) callconv(.c) c_longdouble {
    switch (@typeInfo(c_longdouble).float.bits) {
        16 => return __log1ph(x),
        32 => return log1pf(x),
        64 => return log1p(x),
        80 => return __log1px(x),
        128 => return log1pq(x),
...
test log1pf {
    ...
    try expect(math.approxEqAbs(f32, log1pf(0.0), 0.0, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(0.2), 0.182322, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(0.8923), 0.637793, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(1.5), 0.916291, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(37.45), 3.649359, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(89.123), 4.501175, epsilon));
    try expect(math.approxEqAbs(f32, log1pf(123123.234375), 11.720949, epsilon));
}

test log1p {
    const epsilon = 0.000001;

    try expect(math.approxEqAbs(f64, log1p(0.0), 0.0, epsilon));
    try expect(math.approxEqAbs(f64, log1p(0.2), 0.182322, epsilon));
    try expect(math.approxEqAbs(f64, log1p(0.8923), 0.637793, epsilon));
    try expect(math.approxEqAbs(f64, log1p(1.5), 0.916291, epsilon));
    try expect(math.approxEqAbs(f64, log1p(37.45), 3.649359, epsilon));
    try expect(math.approxEqAbs(f64, log1p(89.123), 4.501175, epsilon));
...
test "log1p_32.special" {
    ...
    try expect(math.isNegativeZero(log1pf(-0.0)));
    try expect(math.isNegativeInf(log1pf(-1.0)));
    try expect(math.isNan(log1pf(-2.0)));
    try expect(math.isNan(log1pf(math.nan(f32))));
}

test "log1p_64.special" {
    try expect(math.isPositiveInf(log1p(math.inf(f64))));
    try expect(math.isPositiveZero(log1p(0.0)));
    try expect(math.isNegativeZero(log1p(-0.0)));
