...
pub fn exp10(val: Value, float_type: Type, arena: Allocator, pt: Zcu.PerThread) !Value {
    const zcu = pt.zcu;
    if (float_type.zigTypeTag(zcu) == .vector) {
        const result_data = try arena.alloc(InternPool.Index, float_type.vectorLen(zcu));
        const scalar_ty = float_type.scalarType(zcu);
        for (result_data, 0..) |*scalar, i| {
            const elem_val = try val.elemValue(pt, i);
            scalar.* = (try exp10Scalar(elem_val, scalar_ty, pt)).toIntern();
        }
        return Value.fromInterned(try pt.intern(.{ .aggregate = .{
            .ty = float_type.toIntern(),
            .storage = .{ .elems = result_data },
    ...
    return exp10Scalar(val, float_type, pt);
}

pub fn exp10Scalar(val: Value, float_type: Type, pt: Zcu.PerThread) Allocator.Error!Value {
    _ = val;
    const zcu = pt.zcu;
    const target = zcu.getTarget();
    const storage: InternPool.Key.Float.Storage = switch (float_type.floatBits(target)) {
        16 => .{ .f16 = 0 }, // @exp10(val.toFloat(f16, zcu)) },
        32 => .{ .f32 = 0 }, // @exp10(val.toFloat(f32, zcu)) },
        64 => .{ .f64 = 0 }, // @exp10(val.toFloat(f64, zcu)) },
        80 => .{ .f80 = 0 }, // @exp10(val.toFloat(f80, zcu)) },
        128 => .{ .f128 = 0 }, // @exp10(val.toFloat(f128, zcu)) },
        else => unreachable,
    };
    return Value.fromInterned(try pt.intern(.{ .float = .{
        .ty = float_type.toIntern(),
...
pub fn log1p(val: Value, float_type: Type, arena: Allocator, pt: Zcu.PerThread) !Value {
    const zcu = pt.zcu;
    if (float_type.zigTypeTag(zcu) == .vector) {
        const result_data = try arena.alloc(InternPool.Index, float_type.vectorLen(zcu));
        const scalar_ty = float_type.scalarType(zcu);
        for (result_data, 0..) |*scalar, i| {
            const elem_val = try val.elemValue(pt, i);
            scalar.* = (try log1pScalar(elem_val, scalar_ty, pt)).toIntern();
        }
        return Value.fromInterned(try pt.intern(.{ .aggregate = .{
            .ty = float_type.toIntern(),
            .storage = .{ .elems = result_data },
    ...
    return log1pScalar(val, float_type, pt);
}

pub fn log1pScalar(val: Value, float_type: Type, pt: Zcu.PerThread) Allocator.Error!Value {
    _ = val;
    const zcu = pt.zcu;
    const target = zcu.getTarget();
    const storage: InternPool.Key.Float.Storage = switch (float_type.floatBits(target)) {
        16 => .{ .f16 = 0 }, // @log1p(val.toFloat(f16, zcu)) },
        32 => .{ .f32 = 0 }, // @log1p(val.toFloat(f32, zcu)) },
        64 => .{ .f64 = 0 }, // @log1p(val.toFloat(f64, zcu)) },
        80 => .{ .f80 = 0 }, // @log1p(val.toFloat(f80, zcu)) },
        128 => .{ .f128 = 0 }, // @log1p(val.toFloat(f128, zcu)) },
        else => unreachable,
    };
    return Value.fromInterned(try pt.intern(.{ .float = .{
        .ty = float_type.toIntern(),
