...
fn genBodyInner(f: *Function, body: []const Air.Inst.Index) error{ AnalysisFail, OutOfMemory }!void {
    ...
    for (body) |inst| {
        ...
        const result_value = switch (air_tags[@intFromEnum(inst)]) {
            ...
            .exp10       => try airUnBuiltinCall(f, inst, air_datas[@intFromEnum(inst)].un_op, "exp10", .none),
            .expm1       => try airUnBuiltinCall(f, inst, air_datas[@intFromEnum(inst)].un_op, "expm1", .none),
            ...
            .log1p       => try airUnBuiltinCall(f, inst, air_datas[@intFromEnum(inst)].un_op, "log1p", .none),
